...
import erc721abi from '../../../lib/blockchain/abis/hedera/erc721.json';
dotenv.config();
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const moonbeamProvider = `${process.env.HEDERATESTNET_NETWORK_PROVIDER}`.split(',')

    const provider = new JsonRpcProvider(moonbeamProvider[0])
    const abi = new Interface(erc721abi);
    const signer = new Wallet(process.env.HEDERA_NFT_ACCOUNT_PRIVATE_KEY, provider);
    // This can be an address or an ENS name
    const address = process.env.HEDERA_NFT721_CONTRACT;
