...
import survey020 from '../../../lib/blockchain/abis/moonbeam/survey020';
dotenv.config();
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const moonbaseProvider = `${process.env.MOONBASEALPHA_NETWORK_PROVIDER}`.split(',')

    const provider = new JsonRpcProvider(moonbaseProvider[0])
    const abi = new Interface(survey020.abi);
    const signer = new Wallet(process.env.MOONBASE_PRIVATE_KEY, provider);
