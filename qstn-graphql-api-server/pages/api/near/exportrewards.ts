...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const contract = await nearContract({
        ...
        account: account,
        viewMethods: ["get_claimable_rewards"],
        changeMethods: ["export_rewards"]
    });
  ...
  try {
    ...
    //if (!response) {
      ...
      const exported_rewards = await contract.export_rewards(creditBalanceParams);
      console.log('exported rewards', exported_rewards);

      //console.log('response ', response);
      res.status(200).json({data: { exported_rewards: exported_rewards }, success: true});
