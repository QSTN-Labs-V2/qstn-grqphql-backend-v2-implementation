...
import erc721abi from '../../../lib/blockchain/abis/polygon/erc721.json';
dotenv.config();
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const polygonProvider = `${process.env.MUMBAI_NETWORK_PROVIDER}`.split(',')

    const provider = new JsonRpcProvider(polygonProvider[0])
    const abi = new Interface(erc721abi);
    const signer = new Wallet(process.env.POLYGON_NFT_ACCOUNT_PRIVATE_KEY, provider);
    // This can be an address or an ENS name
    const address = process.env.POLYGON_NFT721_CONTRACT;
