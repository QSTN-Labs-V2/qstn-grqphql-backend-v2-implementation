...
import erc721abi from '../../../lib/blockchain/abis/opbnb/erc721.json';
dotenv.config();
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const opbnbProvider = `${process.env.BSCTESTNET_NETWORK_PROVIDER}`.split(',')

    const provider = new JsonRpcProvider(opbnbProvider[0])
    const abi = new Interface(erc721abi);
    const signer = new Wallet(process.env.BSCTESTNET_NFT_ACCOUNT_PRIVATE_KEY, provider);
    // This can be an address or an ENS name
    const address = process.env.BSCTESTNET_NFT721_CONTRACT;
  ...
  try {
    ...
    /*erc721rw.on("Transfer", async (to, amount, from) => {
