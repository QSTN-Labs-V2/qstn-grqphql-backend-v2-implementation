...
import survey020 from '../../../lib/blockchain/abis/aurora/survey020';
dotenv.config();
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const auroraProvider = `${process.env.AURORATESTNET_NETWORK_PROVIDER}`.split(',')

    const provider = new JsonRpcProvider(auroraProvider[0])
    const abi = new Interface(survey020.abi);
    const signer = new Wallet(process.env.AURORA_PRIVATE_KEY, provider);
