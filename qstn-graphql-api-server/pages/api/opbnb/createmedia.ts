...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const opbnbProvider = `${process.env.OPBNBTESTNET_NETWORK_PROVIDER}`.split(',')

    const provider = new JsonRpcProvider(opbnbProvider[0])
    const abi = new Interface(erc721abi);
    const signer = new Wallet(process.env.OPBNBTESTNET_NFT_ACCOUNT_PRIVATE_KEY, provider);
    // This can be an address or an ENS name
    const address = process.env.OPBNBTESTNET_NFT721_CONTRACT;
