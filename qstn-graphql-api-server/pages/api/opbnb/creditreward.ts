...
import vault010 from '../../../lib/blockchain/abis/moonbeam/vault010';
dotenv.config();
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const moonbaseProvider = `${process.env.OPBNBTESTNET_NETWORK_PROVIDER}`.split(',')

    const provider = new JsonRpcProvider(moonbaseProvider[0])
    const abi = new Interface(vault010.abi);
    const signer = new Wallet(process.env.OPBNBTESTNET_PRIVATE_KEY, provider);
