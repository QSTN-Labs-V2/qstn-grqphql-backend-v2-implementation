...
import erc721abi from '../../../lib/blockchain/abis/bitfinity/erc721.json';
dotenv.config();
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const avaxProvider = `${process.env.BITFINITYTESTNET_NETWORK_PROVIDER}`.split(',')

    const provider = new JsonRpcProvider(avaxProvider[0])
    const abi = new Interface(erc721abi);
    const signer = new Wallet(process.env.BITFINITYTESTNET_NFT_ACCOUNT_PRIVATE_KEY, provider);
    // This can be an address or an ENS name
    const address = process.env.BITFINITYTESTNET_NFT721_CONTRACT;
  ...
  try {
    ...
    erc721rw.mintItem(userAddress, tokenURI,{
      nonce: await signer.getTransactionCount(),
    }).then(async function (tx) {
      //setMinting(true);
      console.log('Chain ID: ', 355113);
