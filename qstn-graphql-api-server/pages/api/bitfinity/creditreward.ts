...
import vault010 from '../../../lib/blockchain/abis/bitfinity/vault010';
dotenv.config();
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const signer = new Wallet(process.env.BITFINITYTESTNET_PRIVATE_KEY, provider);
  ...
  try {
    ...
    const tx = await contractrw.depositToWallet({value: parseEther(amount), nonce: await signer.getTransactionCount()});
